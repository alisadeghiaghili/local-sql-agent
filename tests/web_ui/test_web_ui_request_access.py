...
"""Regression test for "درخواست دسترسی" (ADR-004 part 1).

``docs/design/DESIGN-INVARIANTS.md`` §8's failure-anatomy table names
"Ask without that column · Request access" as the next actions for a
denied-column guard rejection. This drives the REAL
``web/js/render/turn.js`` (and its full real render dependency chain, same
staging as ``test_web_ui_rejected_sql.py``/``test_web_ui_sql_highlight.py``)
under Node (see ``run_request_access.mjs``) and asserts:

* the button renders beside the existing "ask without column" action, and
  calls ``ctx.onRequestAccess(turnId, column)``;
* it disables itself and relabels while the call is in flight;
* on success it shows the pending sentence, worded differently for a
  fresh request vs. one merged into an already-open one
  (``already_pending``);
* on failure it re-enables with its original label and shows the error;
* a column name containing ``<img src=x onerror=alert(1)>`` renders as
  plain text everywhere -- no ``<img>`` element is ever created;
* the button is offered only when wired, the rejection is genuinely
  ``denied_column``, and there is exactly one column to name.
"""
...
_HARNESS = Path(__file__).resolve().parent / "run_request_access.mjs"

_NODE = shutil.which("node")

# Same rewrite set as test_web_ui_rejected_sql.py's -- see that module for
...
def _subn_or_fail(pattern: re.Pattern[str], replacement: str, text: str, what: str) -> str:
    ...
    assert n == 1, (
        ...
        "update the regex in test_web_ui_request_access.py to match the real source "
...
def _prepare_copies(tmp_path: Path) -> Path:
    ...
    *tmp_path* as ESM (``.mjs``), import paths fixed up. Mirrors
    ``test_web_ui_rejected_sql.py``'s own ``_prepare_copies``."""
    ...
    turn_src = _subn_or_fail(
        ...
        'import { copySourceOfTruth, displaySqlForTurn, formatSqlForDisplay, highlightSql } from "./sql-display.mjs";',
...
def test_request_access_button_wiring_pending_state_and_xss_safety() -> None:
    for p in (
