...
def extract_sql(raw: str) -> str:
    ...
    single = re.match(r"(?i)(SELECT\s+(?:(?!SELECT).)*)", sql, re.DOTALL)
    if single:
        sql = single.group(1).strip()
    sql = " ".join(sql.split())
    sql = re.sub(r"(?i)^SELECT\s+(?!TOP\s+\d+)", "SELECT TOP 100 ", sql)
        ...
        for allowed in ALLOWED_TABLES:
            ...
            if len(parts) == 3:
                ...
                if tbl.upper() == table_name.upper():
                    logger.debug(f"normalize: [BI].[{table_name}] → [{db}].[{schema}].[{tbl}]")
                    return f"[{db}].[{schema}].[{tbl}]"
        logger.warning(f"normalize: [{table_name}] not found in ALLOWED_TABLES.")
        return m.group(0)
...
def validate_sql(sql: str) -> bool:
    ...
    if not upper_sql.startswith("SELECT"):
        logger.warning(f"validate_sql FAIL: Not SELECT → '{upper_sql[:80]}'")
        return False
    if " FROM " not in upper_sql:
        logger.warning("validate_sql FAIL: No FROM clause.")
        return False
    for kw in ("DROP ", "DELETE ", "TRUNCATE ", "ALTER ", "UPDATE ", "INSERT ", "EXEC ", "MERGE "):
        if kw in upper_sql:
            logger.warning(f"validate_sql FAIL: Dangerous keyword → '{kw}'")
            return False
    if " LIMIT " in upper_sql:
        logger.warning("validate_sql FAIL: MySQL LIMIT detected.")
        return False
    found = re.findall(r"\[[A-Za-z0-9_]+\]\.\[[A-Za-z0-9_]+\]\.\[[A-Za-z0-9_]+\]", sql_norm, flags=re.IGNORECASE)
    logger.info(f"validate_sql: Found table refs → {found}")
    if not found:
        logger.warning("validate_sql FAIL: No 3-part table references found.")
        return False
    ...
    if not matched:
        logger.warning(f"validate_sql FAIL: No tables matched ALLOWED_TABLES.\n  Found: {[t.upper() for t in found]}\n  Allowed: {sorted(allowed_set)[:8]} ...")
        return False
...
# =========================================================
def parse_args():
    parser = argparse.ArgumentParser(description="NLQ SQL Assistant - Natural Language to SQL")
    parser.add_argument("--prompt", required=True, type=str, help="Natural language question")
