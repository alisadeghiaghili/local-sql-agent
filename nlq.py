"""nlq.py — unified NLQ entry-point.

Replaces main.py, simple_nlq.py, and prompt_based_nlq.py.
All three shared the same Ollama call + SQL cleanup + execution loop;
the only real difference was the system prompt style.

Usage
-----
    python nlq.py --prompt "top 10 customers by revenue"
    python nlq.py --prompt "..." --mode simple
    python nlq.py --prompt "..." --mode prompt
    python nlq.py --prompt "..." --mode full      # default

Modes
-----
    simple  : minimal system prompt, no schema injection (was simple_nlq.py)
    prompt  : schema-aware prompt loaded from config  (was prompt_based_nlq.py)
    full    : strict rules + allowed-table whitelist  (was main.py)
"""
...
def _clean_sql(raw: str) -> str:
    """Strip markdown fences and keep only the first SELECT/WITH block."""
    ...
    # strip trailing duplicate queries separated by blank line
    sql = sql.split("\n\n")[0].strip()
    # LIMIT → TOP fix
    ...
    # SELECT TOP N DISTINCT → SELECT DISTINCT TOP N
    sql = re.sub(
        r"SELECT\s+TOP\s+(\d+)\s+DISTINCT",
...
def _validate_sql(sql: str) -> None:
    ...
    for kw in ("DELETE ", "UPDATE ", "DROP ", "ALTER ",
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="NLQ → SQL Server  (natural language query assistant)"
    )
